...
## Performance & data pipeline (v109)

The world map loads from `data/world_admin0.topo.json` (both admin-0 layers in one simplified, quantized TopoJSON) and `data/world_capitals.json` — about 0.82 MB gzipped in total, decoded by a small inline reader. Repeat visits are served from the browser cache. If the `data/` folder is missing (for example when running the bare `index.html`), the game falls back to fetching the original Natural Earth GeoJSON remotely.

To regenerate the bundle after a Natural Earth update:

```
npm install topojson-server topojson-simplify topojson-client
node tools/build_world_data.js
```

Then bump `WORLD_DATA_VERSION` in `index.html` so cached copies are invalidated.
